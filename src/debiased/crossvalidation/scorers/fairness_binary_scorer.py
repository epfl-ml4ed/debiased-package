...
class BinaryFairnessScorer(BinaryClfScorer):
    ...
    def __init__(self, mitigating, discriminated, nclasses=2):
        super().__init__({'nclasses': nclasses, 'mitigating': mitigating, 'discriminated': discriminated})
        self._metrics = [
    ...
    def _split_scores(self, y_true: list, y_pred: list, y_probs: list, demographics:list) -> dict:
        metrics = ['tp', 'fp', 'roc', 'recall', 'precision', 'balanced_accuracy', 'roc']
    ...
    def get_fairness_scores(self, y_true:list, y_pred:list, y_probs:list, demographics:list) -> dict:
        ...
        scores = {}
        for attribute in self._settings['mitigating']:
