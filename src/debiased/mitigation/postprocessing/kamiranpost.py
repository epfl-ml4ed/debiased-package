...
from debiased.predictors.predictor import Predictor
from debiased.mitigation.postprocessing.postprocessor import PostProcessor
...
class KamiranPostProcessor(PostProcessor):
    ...
    def __init__(self, mitigating, discriminated, 
        low_threshold=0.01, high_threshold=0.99, num_ROC_margin=50, metric_ub=0.05,
        metric_lb=-0.05, num_class_thresholds=100
    ):
        settings = {
            'mitigating': mitigating, 'discriminated': discriminated, 
            'low_threshold': low_threshold, 'high_threshold': high_threshold, 'num_ROC_margin': num_ROC_margin,
            'metric_ub': metric_ub, 'metric_lb': metric_lb, 'num_class_thresholds': num_class_thresholds
        }
        
        super().__init__(settings)
        self._low_threshold = low_threshold
        self._high_threshold = high_threshold
        self._num_ROC_margin = num_ROC_margin
        self._metric_ub = metric_ub
        self._metric_lb = metric_lb
        self._num_class_thresholds = num_class_thresholds
        self._information = {}

        self._scorer = BinaryFairnessScorer(mitigating, discriminated)
    ...
    def fit_transform( 
        ...
        fair_metric_arr = np.zeros(self._num_class_thresholds*self._num_ROC_margin)
        ...
        for class_thresh in np.linspace(self._low_threshold,
                                        self._high_threshold,
                                        self._num_class_thresholds):
            if class_thresh <= 0.5:
                low_ROC_margin = 0.0
        ...
        rel_inds = np.logical_and(fair_metric_arr >= self._metric_lb,
                                  fair_metric_arr <= self._metric_ub)
        if any(rel_inds):
