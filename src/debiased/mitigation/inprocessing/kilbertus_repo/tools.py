...
import numpy as np
from debiased.mitigation.inprocessing.kilbertus_repo.spfpm import FXnum
...
def to_float(x):
    ...
    if xtype == 'vec':
        return np.array([
            [float(aaa) for aaa in a] for a in x
        ], dtype=float)
    else:
...
def sigmoid(x, approx=None):
    ...
    if _np_instance(x):
        if approx is None or approx in ['none', 'None']:
            if np.sum([int(xx==0) for xx in x]) == len(x):
                return 1
            newx = np.array([float(xx) for xx in x])
